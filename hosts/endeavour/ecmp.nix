{ pkgs, ... }:
{
  systemd = {
    ...
    services.qbittorrent-ecmp-v6 = {
      ...
      serviceConfig = {
        Type = "oneshot";
        ExecStart = pkgs.writeShellScript "qbt-ecmp-v6" ''
          set -euo pipefail
          ...
          for dev in enp2s0 enp4s0; do
            gw="$(${pkgs.iproute2}/bin/ip -6 route show default proto ra dev "$dev" | awk -v d="$dev" '{print $3 "@" d}' | head -n 1)"
            if [ -n "$gw" ]; then
              gws+=("$gw")
            fi
          done
          ...
          if [ "''${#gws[@]}" -eq 1 ]; then
            ${pkgs.iproute2}/bin/ip -6 route replace table 1002 default via "''${gws[0]%@*}" dev "''${gws[0]#*@}"
          else
            ${pkgs.iproute2}/bin/ip -6 route replace table 1002 default \
              nexthop via "''${gws[0]%@*}" dev "''${gws[0]#*@}" weight 1 \
              nexthop via "''${gws[1]%@*}" dev "''${gws[1]#*@}" weight 1
          fi
          ${pkgs.iproute2}/bin/ip -6 route flush cache
        '';
      };
    };
  ...
  networking.sysctl."net.ipv4.fib_multipath_hash_policy" = 1;
}
